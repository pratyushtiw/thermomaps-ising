...
class IsingModel:
    def __init__(self, sampler: Type['Sampler'], size: int, warmup: int, temp: float, Jx: float = 1.0, Jy: float = 1.0):
        ...
        self.trajectory = EnsembleIsingTrajectory(
            summary = Summary(name="IsingModel", size=size, temperature=temp, Jx=Jx, Jy=Jy, ToverJx=temp/Jx, ToverJy=temp/Jy,  Teff = 1 / (Jx/temp +2 ),   sampler=sampler.name),
            state_variables = Summary(temperature=temp, Jx=Jx, Jy=Jy, ToverJx=temp/Jx, ToverJy=temp/Jy, Teff = 1 / (Jx/temp +2 ))
